class TestRecordsController < ApplicationController
  ...
  def index
    ...
    @q = TestRecord.ransack(params[:q])
    @test_records = @q.result(distinct: true)
  end
  ...
  private
    ...
    def test_record_params
      params.require(:test_record).permit(:country, :link, :email, :phone, :name_of_policy, :single_policy_or_group_of_policies, :type_of_policy_instrument, :total_ght_emissions_reductions_in_2020_kt_co2eq_per_year, :realized_net_costs)
    end
end
